...
For 0 {le} _m_ {le} 30, `MDLCK.md[_m_]` locks `SRCMD_PERM(_m_)` and `SRCMD_PERMH(_m_)`, if implemented. For 0 {le} _m_ {le} 31, `MDLCKH.mdh[_m_]` locks `SRCMD_PERM(_m_ + 31)` and `SRCMD_PERMH(_m_ + 31)`, if implemented.

[#Example_Format_2]
.An example block diagram of an SRCMD table implementation in the MD-indexed Format.
