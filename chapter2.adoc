...
For an entry indexed by _i_, `ENTRY_ADDR(_i_)`, `ENTRY_ADDRH(_i_)`, and `ENTRY_CFG(_i_).a` together encode the memory region using the address-matching modes defined by the RISC-V PMP. `ENTRY_ADDR(_i_)` and `ENTRY_ADDRH(_i_)` hold the encoded address for an IOPMP with addresses greater than 34 bits. For addresses less than or equal to 34 bits, only `ENTRY_ADDR(_i_)` is used. To determine whether `ENTRY_ADDRH(_i_)` is implemented, software can check `HWCFG0.addrh_en`. Please refer to <<#HWCFG0,Hardware Configuration 0>> for more details about `HWCFG0.addrh_en`. `ENTRY_CFG(_i_).a` selects the address-matching mode from OFF, NA4, NAPOT, and TOR. Please refer to the RISC-V privileged specification cite:[PrivISA] for details of the encoding schemes. `HWCFG0.tor_en` = 1 indicates that the IOPMP supports the TOR address-matching mode.
...
Any entry with index {ge} `HWCFG1.entry_num` is not available. That is,

* Registers of the entry are not implemented.
* Address-matching mode of the entry is treated as OFF when the IOPMP retrieves the entry in permission checks.
...
[NOTE]
====
A memory domain may have an IOPMP entry with index {ge} `HWCFG1.entry_num` due to its register encoding or implementation. The entry is not available.
====
