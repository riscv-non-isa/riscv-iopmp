...
|===
|Field  |Bits       |R/W    |Default    |Description
|`t`    |15:0       |WARL   |DC/IMP    a| Indicate the upper bound of the IOPMP entry index range of memory domain _m_. If the MDCFG table has a proper setting up to MD _m_, that is, `MDCFG(_k_-1).t` {le} `MDCFG(_k_).t` for every _k_ with 0 < _k_ {le} _m_, an IOPMP entry with index _j_ belongs to MD _m_

- if `MDCFG(_m_-1).t` {le} _j_ < `MDCFG(_m_).t`, where _m_ > 0; or
- if _j_ < `MDCFG(0).t`, where _m_ = 0.

Otherwise, no IOPMP entry belongs to MD _m_; the MDCFG table has an improper setting at or below MD _m_. Please refer to <<#MDCFG_TABLE, MDCFG Table>>.
